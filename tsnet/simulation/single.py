...
def inner_pipe(
    linkp, pn, dt, links1, links2, utype, dtype, p,
    ...
    dVdx: numpy.ndarray
        ...
        in unsteady friction mode [m/s^2]
    dVdt10 : list
        local instantaneous acceleration of left adjacent nodes at
        previous time step [m]
    dVdx10 : list
        convective instantaneous acceleration of left adjacent nodes
        at previous time step [m/s]
    dVdt20 : list
        local instantaneous acceleration of right adjacent nodes at
        previous time step [m]
    dVdx20 : list
        convective instantaneous acceleration of right adjacent nodes
        at previous time step [m/s]
    ...
    if friction == 'steady':
        ...
        H[1:-1], V[1:-1] = inner_node_unsteady(
            linkp, H0, V0, dt, g, dVdx, dVdt
        )
    ...
    if utype[0] == 'Pipe':
        if linkp.start_node.transient_node_type == 'SurgeTank':
            ...
            emitter_coeff = linkp.start_node.emitter_coeff + \
                linkp.start_node.demand_coeff
            block_per = linkp.start_node.block_per
            H[0], V[0] = add_leakage(
                emitter_coeff, block_per, link1, linkp, elev,
                H1, V1, H2, V2, dt, g, 0,  np.sign(links1), [-1],
                friction, dVdx1, dVdx2, dVdt1, dVdt2
            )
    elif utype[0] == 'Pump':
    ...
    if dtype[0] == 'Pipe':
        if linkp.end_node.transient_node_type == 'SurgeTank':
            ...
            emitter_coeff = linkp.end_node.emitter_coeff + \
                linkp.end_node.demand_coeff
            block_per = linkp.end_node.block_per
            H[n], V[n] = add_leakage(
                emitter_coeff, block_per, linkp, link2, elev,
                H1, V1, H2, V2, dt, g, n, [1], np.sign(links2),
                friction, dVdx1, dVdx2, dVdt1, dVdt2
            )
    elif dtype[0] == 'Pump':
...
def left_boundary(
    linkp, pn, H, V, H0, V0, links2, p, pump, valve, dt,
    H20, V20, utype, dtype,
    ...
    dVdx2 = dVdx[0]
    dVdt2 = dVdt[1]
    if utype[0] in ['Reservoir', 'Tank']:
        H[0], V[0] = rev_end(
            H2, V2, H[0], 0, a, g, f, D, dt, KD, friction, dVdx2, dVdt2
        )
    elif utype[0] == 'Valve':
        H[0], V[0] = valve_end(
            H2, V2, V[0], 0, a, g, f, D, dt, KD, friction, dVdx2, dVdt2
        )
    elif utype[0] == 'Junction':
        elev = linkp.start_node.elevation
        H[0], V[0] = dead_end(
            linkp, H2, V2, elev, 0, a, g, f, D, dt, KD, friction, dVdx2, dVdt2
        )
    elif utype[0] == 'Pump':  # source pump
        H[0], V[0] = source_pump(
            pump[0], linkp, H2, V2, dt, g, [-1], friction, dVdx2, dVdt2
        )

    # Pipe end  (inner boundary conditions)
    V1 = V0[n-1]
    H1 = H0[n-1]  # upstream node
    V2 = V20
    H2 = H20  # downstream nodes
    dVdx1 = dVdx[n-1]
    dVdx2 = dVdx20
    dVdt1 = dVdt[n-1]
    dVdt2 = dVdt20
    ...
    if dtype[0] == 'Pipe':
        ...
        if links2 == []:
            H[n], V[n] = valve_end(
                H1, V1, V[n], n, a, g, f, D, dt, KD, friction, dVdx1, dVdt1
            )
        else:
            H[n], V[n] = valve_node(
                valvec, linkp, link2,
        ...
        elev = linkp.end_node.elevation
        H[n], V[n] = dead_end(
            linkp, H1, V1, elev, n, a, g, f, D, dt, KD, friction, dVdx1, dVdt1
        )
...
def right_boundary(
    linkp, pn, H0, V0, H, V, links1, p, pump, valve, dt,
    H10, V10, utype, dtype,
    ...
    dVdx: numpy.ndarray
        ...
        in unsteady friction mode [m/s^2]
    dVdt10 : list
        local instantaneous acceleration of left adjacent nodes
   ...
   Returns
    ...
    V1 = V10
    H1 = H10  # upstream node
    V2 = V0[1]
    H2 = H0[1]  # downstream node
    dVdx1 = dVdx10
    dVdx2 = dVdx[0]
    dVdt1 = dVdt10
    dVdt2 = dVdt[1]
    ...
    dVdx1 = dVdx[n-1]
    dVdt1 = dVdt[n-1]
    if dtype[0] in ['Reservoir', 'Tank']:
        H[n], V[n] = rev_end(
            H1, V1, H[n], n, a, g, f, D, dt, KD, friction, dVdx1, dVdt1
        )
    if dtype[0] == 'Valve':
        H[n], V[n] = valve_end(
            H1, V1, V[n], n, a, g, f, D, dt, KD, friction, dVdx1, dVdt1
