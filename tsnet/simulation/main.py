...
def MOCSimulator(tm, results_obj='results', friction='steady'):
    ...
    if friction not in ['steady', 'unsteady', 'quasi-steady']:
        print(
            "Please specify a friction model from 'steady',"
    ...
    for ts in range(1, tn):
        ...
        if ts == 2:
            for _, pipe in tm.pipes():
                ...
                if abs(diff1) > 1e-1:
                    print(
                        'Initial condition discrepancy of pressure (%.4f m) \
                        on the %s node' % (diff1, pipe.start_node.name)
                    )
                if abs(diff2) > 1e-1:
                    print(
                        'Initial condition discrepancy of pressure (%.4f m) \
                        on the %s node' % (diff2, pipe.end_node.name)
                    )
        if ts == 3:
        ...
        for _, pipe in tm.pipes():
            ...
            if links1[pn] and links2[pn] and \
                ...
                if utype[pn][0] == 'Pump':
                    ...
                    if pipe.start_node.name == \
                            tm.links[utype[pn][1]].start_node.name:
                        pump[0][1] = "s"  # suction side
                    # calculate the coordinate of the three points
                    # based on the pump speed
                    if tm.links[utype[pn][1]].operating is True:
                    ...
                    if tm.links[utype[pn][1]].operating is True:
                        valve[0] = valve_curve(
                            tm.links[utype[pn][1]].operation_rule[ts] * 100,
                            tm.links[utype[pn][1]].valve_coeff
                        )
                    else:
                        # indent level is getting a bit deep here TODO
                        if tm.links[utype[pn][1]].initial_status.name == \
                                'Open':
                            valve[0] = valve_curve(
                                100, tm.links[utype[pn][1]].valve_coeff
                            )
                        elif tm.links[utype[pn][1]].initial_status.name == \
                                'Closed':
                            valve[0] = valve_curve(
                                0, tm.links[utype[pn][1]].valve_coeff
                            )
                # downstream
                if dtype[pn][0] == 'Pump':
                    pump[1] = [tm.links[dtype[pn][1]].curve_coef, "d"]
                    if pipe.end_node.name == \
                            tm.links[dtype[pn][1]].start_node.name:
                        pump[1][1] = "s"  # suction side
                    if tm.links[dtype[pn][1]].operating is True:
                    ...
                    if tm.links[dtype[pn][1]].operating is True:
                        valve[1] = valve_curve(
                            tm.links[dtype[pn][1]].operation_rule[ts] * 100,
                            tm.links[dtype[pn][1]].valve_coeff
                        )
                    else:
                        if tm.links[dtype[pn][1]].initial_status.name == \
                                'Open':
                            valve[1] = valve_curve(
                                100,
                                tm.links[dtype[pn][1]].valve_coeff
                            )
                        elif tm.links[dtype[pn][1]].initial_status.name == \
                                'Closed':
                            valve[1] = valve_curve(
                                0, tm.links[dtype[pn][1]].valve_coeff
                            )
                ...
                if pipe.start_node.transient_node_type == 'Junction':
                    if HN[pn][0] - pipe.start_node.elevation > 0:
                        ...
                        pipe.start_node.demand_discharge[ts] = \
                            pipe.start_node.demand_coeff * np.sqrt(h)
                        pipe.start_node.emitter_discharge[ts] = \
                            pipe.start_node.emitter_coeff * np.sqrt(h)
                    else:  # assume reverse flow preventer installed
                        pipe.start_node.emitter_discharge[ts] = 0.
                        pipe.start_node.demand_discharge[ts] = 0.
                        warnings.warn(
                            "Negative pressure on node %s. Backflow \
                            stopped by reverse flow preventer." %
                            pipe.start_node.name
                        )

                if pipe.end_node.transient_node_type == 'Junction':
                    if HN[pn][-1] - pipe.end_node.elevation > 0:
                        h = HN[pn][-1] - pipe.end_node.elevation
                        pipe.end_node.emitter_discharge[ts] = \
                            pipe.end_node.emitter_coeff * np.sqrt(h)
                        pipe.end_node.demand_discharge[ts] = \
                            pipe.end_node.demand_coeff * np.sqrt(h)
                    else:  # assume reverse flow preventer installed
            ...
            # left boundary pipe
                ...
                if utype[pn][0] == 'Reservoir' or utype[pn][0] == 'Tank':
                    ...
                    pump[0] = [
                        [tm.links[utype[pn][1]].start_node.initial_head][0],
                    ...
                    warnings.warn(
                        'Pipe %s miss %s upstream.' % (pipe, utype[pn][0])
                    )
                ...
                if dtype[pn][0] == 'Pump':
                    ...
                    if tm.links[dtype[pn][1]].operating is True:
                        valve[1] = valve_curve(
                            tm.links[dtype[pn][1]].operation_rule * 100,
                            tm.links[dtype[pn][1]].valve_coeff
                        )
                    else:
                        if tm.links[dtype[pn][1]].initial_status.name == \
                                'Open':
                            valve[1] = valve_curve(
                                100, tm.links[dtype[pn][1]].valve_coeff
                            )
                        elif tm.links[dtype[pn][1]].initial_status.name == \
                                'Closed':
                            valve[1] = valve_curve(
            ...
            #  right boundary pipe
                ...
                # source pump
                    ...
                    pump[1] = [
                        [tm.links[utype[pn][1]].end_node.initial_head][0],
                    ...
                    warnings.warn(
                        'Pipe %s miss %s downstream.' % (pipe, dtype[pn][0])
                    )
                # LEFT boundary
                if utype[pn][0] == 'Pump':
                    pump[0] = [tm.links[utype[pn][1]].curve_coef, "d"]
        ...
        for _, pipe in tm.pipes():
            ...
            if friction == 'unsteady':
                ...
                dVdx[pn] = np.diff(V[pn]) / \
                    (pipe.length/pipe.number_of_segments)
            H[pn] = HN[pn]
            V[pn] = VN[pn]
